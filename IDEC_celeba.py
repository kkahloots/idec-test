...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_clusters', default=40 type=int)
    ...
    if args.dataset == 'mnist':  # recommends: n_clusters=40 update_interval=140
        x, y = load_mnist()
        optimizer = 'adam'
    elif args.dataset == 'usps':  # recommends: n_clusters=40 update_interval=30
        x, y = load_usps('data/usps')
    elif args.dataset == 'reutersidf10k':  # recommends: n_clusters=40, update_interval=3
        x, y = load_reuters('data/reuters')

    # prepare the IDEC model
    idec = IDEC(dims=[x.shape[-1], 250, 250, 500, 40], n_clusters=args.n_clusters, batch_size=args.batch_size)
